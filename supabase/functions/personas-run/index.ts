...
// Selecting more than this many at once would fan out too many parallel Claude calls.
...
// Read the confirmed Step-1 research brief (job row first, workspace copy as fallback) so personas
// can ground real customers / competitors / proof instead of inventing them.
...
async function runPipeline(sb: SupabaseClient, anthropicKey: string, wsId: string, userContext = "", icpIds: string[] = []): Promise<void> {
  ...
  // The user curates which ICPs become personas on the Step 4 gate (ICP picker). When a selection
  // is provided, build personas for exactly those (in score order); otherwise default to the top N.
  let targets: any[];
  if (Array.isArray(icpIds) && icpIds.length) {
    const wanted = new Set(icpIds);
    targets = ordered.filter((i) => wanted.has(i.id)).slice(0, MAX_SELECTABLE);
    await appendLog(sb, wsId, `Using ${targets.length} user-selected ICP(s).`);
  } else {
    targets = ordered.slice(0, MAX_PERSONAS);
  }

  // Grounding facts from the confirmed research brief — real customers/competitors only.
  const cbrief = await readResearchBrief(sb, wsId) || cd._initialResearchBrief || {};
  const cpos = cbrief.competitivePositioning || {};
  const ctme = cbrief.targetMarketEvidence || {};
  const realCompetitors = (Array.isArray(cpos.mainCompetitors) ? cpos.mainCompetitors.join(", ") : "") || cd.co_competitors || "";
  const realCustomers = (Array.isArray(ctme.knownCustomers) ? ctme.knownCustomers.filter(Boolean).join(", ") : "") || cd.co_customers || "";
  ...
  const personas = await Promise.all(targets.map(async (icp: any) => {
    const personaPrompt = `Draft a B2B persona for cold outreach.
...
serve(async (req: Request) => {
  ...
  try {
    const { workspaceId, userContext, icpIds } = await req.json() as { workspaceId?: string; userContext?: string; icpIds?: string[] };
    ...
    EdgeRuntime.waitUntil((async () => {
      try { await runPipeline(sb, anthropicKey, workspaceId, userContext || "", Array.isArray(icpIds) ? icpIds : []); }
      catch (err) {
